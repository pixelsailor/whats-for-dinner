  ...
  <Tooltip.Provider delayDuration={300} ignoreNonKeyboardFocus={true}>
    <Tooltip.Root {open}>
      ...
      <Tooltip.Portal>
        <Tooltip.Content
          sideOffset={4}
          class="text-helper-text rounded-sm border border-gray-600 bg-gray-600/80 px-2 text-gray-50 shadow-md dark:border-gray-300 dark:bg-gray-300/80 dark:text-gray-900"
        >
...
{:else}
  <Button.Root class="button icon text" {...props}>
    {@render children()}
  </Button.Root>
{/if}
