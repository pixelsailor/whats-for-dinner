...
| **Authority transfer** for [ADR-008 § Implementation compliance](../adrs/ADR-008-schema-led-domain-contracts.md#implementation-compliance-discovered) citing `agents.md` for Zod / `z.infer` discipline | **Already covered** by [`.cursor/rules/schema-and-type-safety.mdc`](../.cursor/rules/schema-and-type-safety.mdc) (resolved **GAP-007** in this file). `lint-and-code-quality.mdc` cross-links to that rule. ADR rewording remains the open todo "Update ADRs that cite `agents.md`" in [`docs/adr-and-rules-todo.md`](./adr-and-rules-todo.md). |
