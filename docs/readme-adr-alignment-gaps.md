...
- **Status:** Open
- **Severity:** Minor
- **Source:** [agents.md](../agents.md) — _Architecture highlights_ (OpenAI integration); [ADR-007](../adrs/ADR-007-ai-provider-contract.md)
...
- **Expected:** Full module-location and stack guidance should live in Cursor rules per [`adr-and-rules-todo.md`](./adr-and-rules-todo.md) _Legacy `agents.md` Distribution_.
...
Migrating _TypeScript Conventions_, _General Code Style_, and _Accessibility and UX_ from [`agents.md`](../agents.md) into [`.cursor/rules/lint-and-code-quality.mdc`](../.cursor/rules/lint-and-code-quality.mdc) (rows 7, 12, 13 in [`docs/agents-md-distribution-worksheet.md`](./agents-md-distribution-worksheet.md)) did not open new product gaps. It **documented agent guidance** and reconciled internal contradictions:

| Topic | Resolution / where tracked |
| ----- | -------------------------- |
| **Indentation contradiction** in `agents.md` (_General Code Style_: "Use 2-space indentation" vs _TypeScript Conventions_: "Respect indents. Keep nested items aligned. Do not reset tabs for nested content.") | **Resolved by deferring to [`.prettierrc`](../.prettierrc) as the source of truth** — the "Use 2-space indentation" wording contradicted both the configured formatter and the codebase. Sample files (e.g. [`src/lib/api/recipe/recipe.schemas.ts`](../src/lib/api/recipe/recipe.schemas.ts)) already match Prettier. [`.cursor/rules/lint-and-code-quality.mdc`](../.cursor/rules/lint-and-code-quality.mdc) and the [`agents.md`](../agents.md) mirror now point at `.prettierrc` rather than restating values, so future config changes do not require rule edits. |
| **JSDoc overlap** between _General Code Style_ ("Always comment complex logic with short, meaningful JSDoc-style notes") and [`.cursor/rules/documentation-conventions.mdc`](../.cursor/rules/documentation-conventions.mdc) (enterprise-grade JSDoc on production TS, Svelte `@component` blocks) | **No conflict.** `lint-and-code-quality.mdc` defers to `documentation-conventions.mdc` for what gets a doc block; the legacy "comment complex logic" bullet stays in `agents.md` as a mirror summary, not as authority. |
| **Accessibility duplication** between `agents.md` _Accessibility and UX_, [`.cursor/rules/svelte-5-ui-conventions.mdc`](../.cursor/rules/svelte-5-ui-conventions.mdc), and [ADR-014](../adrs/ADR-014-semantic-html-and-accessibility.md) (Accepted) | **A11y stays under ADR-014 and the Svelte rule** (authoritative). Cross-cutting bullets that apply outside `.svelte` — async loading/error states for AI/Dexie/sync work, "clarity over cleverness" — moved into a small _Accessibility (cross-link)_ section of `lint-and-code-quality.mdc` so non-Svelte TypeScript code is not orphaned. ADR-014 motion / `prefers-reduced-motion` requirement replaces the looser "animations are subtle and never block user interaction" phrasing. |
| **ESLint import-order claim** in `agents.md` ("Keep imports ordered: external → internal → local") vs configured ESLint (`sort-imports` with `ignoreDeclarationSort: true`) | **Documented, not auto-enforced.** Declaration order is a social / review convention today; member sort is configured. `lint-and-code-quality.mdc` notes this explicitly so agents do not assume ESLint will reorder declarations for them. **Not a product gap**; revisit if the team decides to adopt an `import/order`-style rule. |
| **Authority transfer** for [ADR-008 § Implementation compliance](../adrs/ADR-008-schema-led-domain-contracts.md#implementation-compliance-discovered) citing `agents.md` for Zod / `z.infer` discipline | **Resolved:** [`.cursor/rules/schema-and-type-safety.mdc`](../.cursor/rules/schema-and-type-safety.mdc) ( **GAP-007** ); ADR-002, ADR-006, and ADR-008 no longer cite `agents.md` as authority ([`docs/adr-and-rules-todo.md`](./adr-and-rules-todo.md) **Update ADRs that cite `agents.md`**). |

**No new GAP rows added.** Drift this pass touched was either documentation reconciliation (indentation, a11y home) or already tracked under existing IDs (**GAP-011** for `agents.md` retirement, **GAP-018** for ADR enforcement-text catch-up).
...
| ADR-006 **Enforcement rules** cited [`agents.md`](../agents.md) as transitional authority | **Resolved (2026-05-16):** ADR-006 cites [`.cursor/rules/serverless-compatibility.mdc`](../.cursor/rules/serverless-compatibility.mdc) only; [`agents.md`](../agents.md) retirement remains [`docs/adr-and-rules-todo.md`](./adr-and-rules-todo.md) **Retire `agents.md`** |
