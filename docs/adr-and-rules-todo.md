...
- [x] **Migrate Dexie + LiveQueryStores guidelines**
  - Move *Dexie + LiveQueryStores Guidelines* into **Rule: Local data and Dexie ownership** ([`.cursor/rules/local-data-dexie-ownership.mdc`](../.cursor/rules/local-data-dexie-ownership.mdc)), with cross-references to [ADR-002](../adrs/ADR-002-local-data-ownership.md).
	...
	- **Done:** rule sections *Reads and LiveQuery* and *Conventions* expanded (`liveQuery` vs `createLiveQueryStore`, derived views, Zod row types, minimal example).
...
- [x] **Migrate Supabase auth, sharing, and cloud-backup wiring**
  ...
  - **Done:** [`.cursor/rules/supabase-enhancement-boundary.mdc`](../.cursor/rules/supabase-enhancement-boundary.mdc) now includes *Auth, sharing, and backup — client surfaces* (accurate map: `locals.supabase`, `+layout.ts` / `data.supabase`, narrow `supabaseClient.ts`). **Surfaced discrepancies:** [Deferred — Supabase wiring migration notes](./readme-adr-alignment-gaps.md#supabase-wiring-migration-notes-2026-05-12) (GAP-016 / GAP-017).
...
- [x] **Update ADRs that cite `agents.md`**
  - **Done (2026-05-16):** [ADR-002](../adrs/ADR-002-local-data-ownership.md) (*Supporting context*) now cites README, ADR-008, and [`.cursor/rules/schema-and-type-safety.mdc`](../.cursor/rules/schema-and-type-safety.mdc) / [`.cursor/rules/local-data-dexie-ownership.mdc`](../.cursor/rules/local-data-dexie-ownership.mdc). [ADR-006](../adrs/ADR-006-serverless-and-secret-boundary.md) (*Decision pressure*, *Enforcement rules*) is self-contained and cites [`.cursor/rules/serverless-compatibility.mdc`](../.cursor/rules/serverless-compatibility.mdc). [ADR-008](../adrs/ADR-008-schema-led-domain-contracts.md) already cited the schema rule (no `agents.md` remained).
