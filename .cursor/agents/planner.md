...
8. For medium/large runs or any testable code change, copy [`.cursor/orchestrations/_template/test-matrix.md`](../orchestrations/_template/test-matrix.md) and fill **Planned coverage** using layer IDs from [`docs/test-matrix-template.md`](../../docs/test-matrix-template.md). For small doc-only or explicitly non-testable runs, `test-matrix.md` is optional.

## Rules

1. The Planner MUST produce both `plan.md` and `acceptance-criteria.md` under `.cursor/orchestrations/{task-id}/` before handoff.
2. `plan.md` MUST contain every section listed in the Output Contract, in order. Section headings MUST match the template so Orchestrator, Builder, Tester, and Validator can rely on stable anchors.
...
5. Each acceptance criterion MUST be independently verifiable by Tester or Validator without inferring unstated intent.
...
13. The Planner MUST define test intent for each AC: automated, manual, or explicitly uncovered until the Tester decides final implementation.
...
17. **`test-matrix.md`:** Required for `medium`/`large` runs or any testable code change; optional for `small` doc-only or explicitly non-testable runs. When required, fill **Planned coverage** only (Tester completes **Actual coverage**).
...
3. **`.cursor/orchestrations/{task-id}/test-matrix.md`** — **Required** for `medium`/`large` runs or any testable code change; **optional** for `small` doc-only or explicitly non-testable runs. **Planned coverage** only; Tester completes **Actual coverage**. Canonical shape: [`.cursor/orchestrations/_template/test-matrix.md`](../orchestrations/_template/test-matrix.md); layer definitions: [`docs/test-matrix-template.md`](../../docs/test-matrix-template.md).
