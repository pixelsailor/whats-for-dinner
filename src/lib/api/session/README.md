...
GAP-017 required **one documented surface** for _how_ clients are built at boundaries (`@supabase/ssr` cookie adapters, layout `fetch`, env keys) — not new auth, account, or cloud **business logic**. That is **session transport** wiring, so it lives here under the **session** domain name.
