...
- **Observed:**
  - **Historical:** `PUBLIC_SUPABASE_PUBLISHABLE_KEY` was used in [`src/routes/+layout.ts`](../src/routes/+layout.ts), while `PUBLIC_SUPABASE_ANON_KEY` was used in [`src/hooks.server.ts`](../src/hooks.server.ts), [`src/lib/supabaseClient.ts`](../src/lib/supabaseClient.ts), `src/lib/db/remote.ts` (since **removed**; recipe cloud I/O lives under [`src/lib/api/cloud/`](../src/lib/api/cloud/)), and ~~[`src/lib/api/common/common.model.ts`](../src/lib/api/common/common.model.ts)~~ (**removed**, COM-3).
...
- **Observed:** ~~Supabase JS clients are still created in **multiple disconnected ways** (`@supabase/ssr` in `hooks.server.ts` and `+layout.ts`, plain `createClient` in `supabaseClient.ts`).~~ **Resolved:** [`src/lib/api/session/`](../src/lib/api/session/) centralizes cookie-backed factories; [`createAnonymousCloudClient`](../src/lib/api/cloud/cloud.client.ts) covers anonymous share-by-token reads; `hooks.server.ts` and `+layout.ts` import session factories; standalone `src/lib/supabaseClient.ts` removed.
...
- **Notes:** **2026-06-05 resolution:** session factories in [`src/lib/api/session/README.md`](../src/lib/api/session/README.md); anonymous cloud client in `cloud.client.ts`; Vitest in `session.model.test.ts` and `cloud.client.test.ts`. Overlaps items (2)–(3) in [Deferred — Supabase consolidation audit](#supabase-consolidation-audit-2026-05-10).
...
- **Observed:** [`importRecipeFromURL`](../src/lib/api/ai/ai.server.service.ts) sent only `The URL is: …` to OpenAI; [`/api/import/url`](../src/routes/api/import/url/+server.ts) did not fetch the page.
...
- **Notes:** Remediated 2026-05-31 via `src/lib/api/recipe-import/` and route orchestration per ADR-017.
- **Owner:** —
...
| **Permission flags**    | `ai_assistance`, `read_cloud`, and `write_cloud` from `user_profiles`, cached in **httpOnly cookie** after login | `AccountService` in `auth/+page.server.ts`; `getSessionPermissions` / `setSessionPermissions` in [`src/lib/api/auth/auth.permissions.ts`](../src/lib/api/auth/auth.permissions.ts); exposed as `data.permissions.cloudRead` / `cloudWrite` in [`+layout.server.ts`](../src/routes/+layout.server.ts) |
| **Browser “online”**    | `navigator.onLine` only — **not** a probe of Supabase reachability                                               | [`src/lib/stores/network.ts`](../src/lib/stores/network.ts); gates sync in [`+layout.svelte`](../src/routes/+layout.svelte) (`runSync` bails if offline or neither cloud flag)                                                                                                                       |
| **AI capability UI**    | Combines online + `featureFlags.openai` + session + `aiAssistedRecipe` permission                                | [`src/lib/api/auth/auth.capability.ts`](../src/lib/api/auth/auth.capability.ts) `deriveAICapability`                                                                                                                                                                                                 |
| **Cloud capability UI** | Combines online + session + `cloudRead` / `cloudWrite`                                                           | [`src/lib/api/auth/auth.capability.ts`](../src/lib/api/auth/auth.capability.ts) `deriveCloudCapability` — used by layout sync; recipe pages still gate writes on `cloudWrite.allowed`                                                                                                                |

**Implication:** A user can be “online” per the browser but still fail cloud calls (DNS, 401, RLS); sync currently surfaces errors via try/catch and `syncStore`, not via a unified “cloud reachable” signal. **Proposed direction:** keep `navigator.onLine` as a cheap gate, optionally add explicit Supabase health or failed-call backoff in `SyncService` / layout. `deriveCloudCapability` covers session + `read_cloud`/`write_cloud` + online symmetry with AI; fuller GAP-010 parity (feature flags, every page) remains optional.

#### Supabase client entry points (inventory)

| #   | Location                                                                                      | Client type                                                                                                    | Session / cookies                                                                                    | Still used?                                                                                                                                                                                                                                                                                  |
| --- | --------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | [`src/hooks.server.ts`](../src/hooks.server.ts)                                               | `createRequestServerClient` via [`src/lib/api/session/`](../src/lib/api/session/)                              | Yes — canonical trusted SSR auth/session path                                                        | **Yes** — **GAP-016** / **GAP-017** resolved (2026-06-05)                                                                                                                                                                                                                                    |
| 2   | [`src/routes/+layout.ts`](../src/routes/+layout.ts)                                           | `createLayoutBrowserClient` / `createLayoutServerClient` via [`src/lib/api/session/`](../src/lib/api/session/) | Yes (SSR branch reads cookies from layout data); trusted session/user comes from `+layout.server.ts` | **Yes** — **GAP-017** resolved (2026-06-05)                                                                                                                                                                                                                                                  |
| 3   | ~~[`src/lib/supabaseClient.ts`](../src/lib/supabaseClient.ts)~~ **removed**                   | Was `createClient` singleton                                                                                   | No                                                                                                   | **No** — replaced by `createAnonymousCloudClient` in [`src/lib/api/cloud/cloud.client.ts`](../src/lib/api/cloud/cloud.client.ts) (share-by-token only)                                                                                                                                       |
...
| 5   | ~~[`src/lib/api/common/common.model.ts`](../src/lib/api/common/common.model.ts)~~ **removed** | Was standalone `createClient` + `PUBLIC_SUPABASE_PUBLISHABLE_KEY`                                              | No                                                                                                   | **No** — deleted (COM-3); see [`src/lib/api/common/README.md`](../src/lib/api/common/README.md)                                                                                                                                                                                              |
...
**Factory consolidation:** `hooks.server.ts` and `+layout.ts` still construct distinct SSR clients per the `@supabase/ssr` SvelteKit pattern, but both route through [`src/lib/api/session/`](../src/lib/api/session/) factories (**GAP-017** resolved 2026-06-05). All clients use `PUBLIC_SUPABASE_PUBLISHABLE_KEY`.

**Proposed consolidation (phased):**

1. ~~**Verify (GAP-016):** Run the **browser acceptance** checklist after the publishable-key migration before closing the gap.~~ **Done (2026-06-05).**
2. ~~**Unified client factory (GAP-017):** Replace ad hoc `createClient` / duplicate SSR setup with one documented internal surface.~~ **Done (2026-06-05):** [`src/lib/api/session/`](../src/lib/api/session/) + [`cloud.client.ts`](../src/lib/api/cloud/cloud.client.ts).
3. **Remove dead paths:** `src/lib/db/remote.ts` **removed** (superseded by `src/lib/api/cloud/`). ~~`common.model.ts`~~ **removed** (COM-3).
4. **Capability matrix in layout data (aligns with GAP-010):** Prefer `deriveCloudCapability` (session + `read_cloud`/`write_cloud` + online) for sync/UI gates, mirroring `deriveAICapability`. Layout sync already uses it; remaining recipe pages may adopt it opportunistically instead of re-deriving `cloudWrite.allowed` alone.
...
**Maintainer position:** these env vars corresponded to **two different keys** issued by Supabase (not duplicate names for one paste). **`PUBLIC_SUPABASE_ANON_KEY` was obsolete** for forward-looking work; production source now uses **`PUBLIC_SUPABASE_PUBLISHABLE_KEY`**. **GAP-016** and **GAP-017** both closed (2026-06-05).
...
| **Dual public keys and multiple client factories**                             | **GAP-016** and **GAP-017** resolved (2026-06-05): publishable key only; session factories in [`src/lib/api/session/`](../src/lib/api/session/) ([`.cursor/rules/supabase-enhancement-boundary.mdc`](../.cursor/rules/supabase-enhancement-boundary.mdc)).                                                                                                                                               |
...
| Dual public Supabase keys and mixed client factories                         | **GAP-016** and **GAP-017** resolved (2026-06-05); **Supabase consolidation audit** above |
...
| **GAP-016** | Dual Supabase public keys consolidated on **`PUBLIC_SUPABASE_PUBLISHABLE_KEY`** in all production client entry points (`hooks.server.ts`, `+layout.ts`, `supabaseClient.ts`); `PUBLIC_SUPABASE_ANON_KEY` removed from env; browser-verified auth, session refresh, layout-driven auth, and cloud flows (2026-06-05). Factory sprawl tracked separately under **GAP-017**.                                                                                                                                                                                                                                                                                                                                                                                                                         |
| **GAP-017** | Session client factories in [`src/lib/api/session/`](../src/lib/api/session/); anonymous reads via `createAnonymousCloudClient` in [`cloud.client.ts`](../src/lib/api/cloud/cloud.client.ts); `hooks.server.ts`, `+layout.ts`, and share-by-token route wired; `src/lib/supabaseClient.ts` removed; Vitest `session.model.test.ts`, `cloud.client.test.ts` (2026-06-05).                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **GAP-026** | Domain-adjacent helpers migrated from `$lib/utils`: permission cookies → [`auth.permissions.ts`](../src/lib/api/auth/auth.permissions.ts); AI capability policy → [`auth.capability.ts`](../src/lib/api/auth/auth.capability.ts); prompt sanitization/intent heuristics → [`ai.model.ts`](../src/lib/api/ai/ai.model.ts); `sentenceCase` → [`utils/sentenceCase.ts`](../src/lib/utils/sentenceCase.ts). Removed misplaced `utils/session.ts` and `utils/capabilities.ts` plus the legacy single-file utils barrel. Vitest: `auth.capability.test.ts`, `auth.permissions.test.ts` (2026-06-05).                                                                                                                                                                                                    |
