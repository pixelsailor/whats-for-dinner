...
Migrating *TypeScript Conventions*, *General Code Style*, and *Accessibility and UX* from [`agents.md`](../agents.md) into [`.cursor/rules/lint-and-code-quality.mdc`](../.cursor/rules/lint-and-code-quality.mdc) (rows 7, 12, 13 in [`docs/agents-md-distribution-worksheet.md`](./agents-md-distribution-worksheet.md)) did not open new product gaps. It **documented agent guidance** and reconciled internal contradictions:

| Topic | Resolution / where tracked |
| ----- | -------------------------- |
| **Indentation contradiction** in `agents.md` (*General Code Style*: "Use 2-space indentation" vs *TypeScript Conventions*: "Respect indents. Keep nested items aligned. Do not reset tabs for nested content.") | **Resolved by deferring to [`.prettierrc`](../.prettierrc) as the source of truth** — the "Use 2-space indentation" wording contradicted both the configured formatter and the codebase. Sample files (e.g. [`src/lib/api/recipe/recipe.schemas.ts`](../src/lib/api/recipe/recipe.schemas.ts)) already match Prettier. [`.cursor/rules/lint-and-code-quality.mdc`](../.cursor/rules/lint-and-code-quality.mdc) and the [`agents.md`](../agents.md) mirror now point at `.prettierrc` rather than restating values, so future config changes do not require rule edits. |
| **JSDoc overlap** between *General Code Style* ("Always comment complex logic with short, meaningful JSDoc-style notes") and [`.cursor/rules/documentation-conventions.mdc`](../.cursor/rules/documentation-conventions.mdc) (enterprise-grade JSDoc on production TS, Svelte `@component` blocks) | **No conflict.** `lint-and-code-quality.mdc` defers to `documentation-conventions.mdc` for what gets a doc block; the legacy "comment complex logic" bullet stays in `agents.md` as a mirror summary, not as authority. |
| **Accessibility duplication** between `agents.md` *Accessibility and UX*, [`.cursor/rules/svelte-5-ui-conventions.mdc`](../.cursor/rules/svelte-5-ui-conventions.mdc), and [ADR-014](../adrs/ADR-014-semantic-html-and-accessibility.md) (Accepted) | **A11y stays under ADR-014 and the Svelte rule** (authoritative). Cross-cutting bullets that apply outside `.svelte` — async loading/error states for AI/Dexie/sync work, "clarity over cleverness" — moved into a small *Accessibility (cross-link)* section of `lint-and-code-quality.mdc` so non-Svelte TypeScript code is not orphaned. ADR-014 motion / `prefers-reduced-motion` requirement replaces the looser "animations are subtle and never block user interaction" phrasing. |
