...
The root [`agents.md`](../agents.md) predates the ADR system and the `.cursor/rules/` layout. Its content (product framing, Svelte 5 + runes guidance, TypeScript conventions, Dexie + LiveQueryStores patterns, TanStack Query usage, Supabase auth wiring, Zod rules, general code style, accessibility, architecture highlights, env/secrets, doc references) is still useful but should be **distributed** to ADRs, Cursor rules, and scoped READMEs. Accepted ADRs [ADR-002](../adrs/ADR-002-local-data-ownership.md), [ADR-006](../adrs/ADR-006-serverless-and-secret-boundary.md), and [ADR-008](../adrs/ADR-008-schema-led-domain-contracts.md) no longer cite `agents.md` as authority; remaining work is to retire the file and update other repo references (for example [ADR-001](../adrs/ADR-001-product-operating-model.md) enforcement text per **GAP-018**). Preserve `agents.md` as-is until each task below is complete; alignment gaps surfaced during the audit are tracked in [`readme-adr-alignment-gaps.md`](./readme-adr-alignment-gaps.md).
