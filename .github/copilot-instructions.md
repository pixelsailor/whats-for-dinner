...
- Cloud sync uses Supabase for authorized users via `src/lib/supabaseClient.ts` (PUBLIC*SUPABASE*\* envs).
...
- Public keys: use `PUBLIC_*` env vars for values safe to expose (Supabase URL/publishable key found in
