...
8. **Source tree boundaries** under `src/lib/` (see also [`src/lib/utils/README.md`](../src/lib/utils/README.md)):
   ...
   | **`src/lib/utils/`**        | **General-purpose helpers** reused across unrelated features: pure functions with **no owning domain** (e.g. date parsing, secure random bytes, generic string casing). One concern per file; import from `$lib/utils/<name>.ts`. See [`src/lib/utils/README.md`](../src/lib/utils/README.md). | Auth/session/permission logic, AI prompt handling, Supabase I/O, recipe/cloud/account contracts, or anything that would break `$lib/api/*` organization if added here. **`$lib/utils` is not a catch-all.** |
...
| **`$lib/utils` vs `$lib/api` drift** | **Resolved (GAP-026, 2026-06-05):** permission cookies, AI capability policy, and prompt helpers moved to `$lib/api/auth/` and `$lib/api/ai/`; `$lib/utils/` holds domain-agnostic helpers only (one file per concern).                                                                                   |
...
- **Code / architecture:** Domain entities and API payloads live under `src/lib/api/**` with the `schemas` / `types` / `model` / `service` split; Dexie entity generics import from API barrels; no new parallel domain `type` blocks in `$lib/types.ts`. General helpers belong in `src/lib/utils/<name>.ts` only when domain-agnostic (ADR-008 §8).
