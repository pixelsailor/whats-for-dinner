...
- [ ] **Fix the pipeline order wording drift.**
...
- [ ] **Clarify whether the final Orchestrator is part of `pipeline`.**
...
- [ ] **Standardize `test-matrix.md` as optional, recommended, or required.**
...
- [ ] **Resolve `human-approval.md` optional vs required language.**
...
- [ ] **Align `build-log.md` template with the Builder output contract.**
...
- [ ] **Align `test-report.md` template with the Test output contract.**
...
- [ ] **Fix duplicated numbering in the Validator inputs.**
...
- [ ] **Define the small-run completion path.**
...
- [ ] **Rename or reframe the merge-ready gate vocabulary.**
...
- [ ] **Create a single precedence rule for conflicts.**
...
- [ ] **Fix relative links in `orchestration-artifacts.mdc`.**
...
- [ ] **Reconcile `.cursor/rules/index.md` with orchestration references.**
...
- [ ] **Validate agent model frontmatter against current Cursor model names.**
...
- [ ] **Reduce repeated gate explanations.**
...
- [ ] **Shorten the prompt library or move it into role-owned snippets.**
...
- [ ] **Choose a less ambiguous name for the `Test` role.**
  ...
  - Targets: `.cursor/agents/test.md`, `.cursor/agents/INDEX.md`, `docs/ORCHESTRATED_DEVELOPMENT.md`, `.cursor/rules/test-matrix.mdc`, `docs/test-matrix-template.md`, `.cursor/orchestrations/_template/*`.

- [ ] **Normalize task id terminology.**
...
- [ ] **Define allowed `gate_status` values.**
...
- [ ] **Clarify `PASS_WITH_NOTES` semantics.**
...
- [ ] **Clarify whether `pnpm run build` is default evidence or task-specific evidence.**
...
- [ ] **Simplify repeated role responsibility tables.**
...
- [ ] **Add a short "minimum read set" for each role.**
...
- [ ] **Decide whether orchestration task folders are normally committed.**
