...
- **Out of scope:** Supabase sync, conflict resolution, and cloud row lifecycle ([future ADR on sync and conflict resolution](../adr-and-rules-todo.md)); OpenAI request/response contracts and suggestion **lifecycle** policy beyond storage classification ([future ADR on AI suggestion lifecycle](../adr-and-rules-todo.md)); which UI paths may **call** AI or require auth ([ADR-001](ADR-001-product-operating-model.md) capability matrix); Zod schema placement ([future schema-led ADR](../adr-and-rules-todo.md)); exact table migrations and versioned Dexie schemas (implementation detail, governed here only at the ownership level).
