...
**Typical signed-in path:** session factories supply the client → auth establishes validated identity → account loads profile flags → layout/route surfaces cached permission flags for enhancement gating (httpOnly cookie helpers in [`src/lib/api/auth/auth.permissions.ts`](../src/lib/api/auth/auth.permissions.ts)). **Cloud** ([`CloudService`](../src/lib/api/cloud/cloud.service.ts), [`SyncService`](../src/lib/api/cloud/sync.service.ts)) uses the same injected client for backup/sync/share — not the session module.
