...
5. **Each feature owns its design doc**: A per-feature doc (or ADR successor) lives under `docs/<feature>/...` and is linked from this ADR's notes when written. The README *Roadmap Direction* bullet remains the one-liner; the deep design lives elsewhere.
...
| **Deeper design home** | `docs/recipe-import-url/...` (to be created). |
